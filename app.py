...
class StockMarketAnalysis:
    ...
    @st.cache_data
    def evaluate_sarima_model(self, series, order, seasonal_order, _period):
        # Limit data to last 3 years (approx. 756 trading days) to reduce computation
        max_data_points = 756
        if len(series) > max_data_points:
            series = series[-max_data_points:]
            logging.info(f"Reduced SARIMA data to last {max_data_points} points for {self.symbol}")
        ...
        try:
            model = SARIMAX(
                ...
                maxiter=30,  # Reduced iterations for faster convergence
                method='powell'  # Simpler solver for speed
            )
            ...
            with tab4:
                ...
                with st.spinner("Evaluating SARIMA model (this may take a moment)..."):
                    try:
                        ...
                        # Use simpler seasonal order to reduce computation
                        sarima_rmse = analysis.evaluate_sarima_model(
                            analysis.data['Close'],
                            order=best_order,
                            seasonal_order=(0, 0, 0, period),  # Simplified seasonal component
                            _period=period
                        )
                        st.write(f"SARIMA{best_order}(0,0,0)[{period}] RMSE: {sarima_rmse:.3f}")
                        st.write(f"**Interpretation**: SARIMA extends ARIMA with seasonal components. Using a simplified seasonal order reduces computation time while capturing trends. A lower RMSE suggests a good fit.")
                    except Exception as e:
                ...
                try:
                    best_order = st.session_state.grid_search_result['order'] if st.session_state.grid_search_status == "completed" and st.session_state.grid_search_result else (0, 1, 0)
                    model_comparison = {
                        "Model": [f"ARIMA{best_order}", f"SARIMA{best_order}(0,0,0)[{period}]", "TSLM"],
