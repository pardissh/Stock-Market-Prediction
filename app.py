...
# Update to StockMarketAnalysis class load_data method
class StockMarketAnalysis:
    def __init__(self, symbol, start_date, end_date, cutoff_date="2024-12-31"):
        self.symbol = symbol
    ...
    def load_data(self):
        ...
        cutoff_date = pd.Timestamp(self.cutoff_date).tz_localize('UTC')
        current_date = pd.Timestamp.now(tz='UTC').normalize()  # Normalize to midnight UTC
        yfinance_max_date = pd.Timestamp("2025-04-30").tz_localize('UTC')  # Max date from CSV generation
        historical_file = f"{self.symbol.lower()}_historical.csv"

        # Log working directory and CSV file path
        working_dir = os.getcwd()
        absolute_file_path = os.path.abspath(historical_file)
        logging.info(f"Current working directory: {working_dir}")
        logging.info(f"Looking for CSV file at: {absolute_file_path}")

        # Adjust end_date to min(current_date, yfinance_max_date) to avoid unavailable data
        if end_date > current_date:
            logging.info(f"End date {end_date.date()} exceeds current date {current_date.date()}. Adjusting to {current_date.date()}.")
            end_date = current_date
        if end_date > yfinance_max_date:
            logging.info(f"End date {end_date.date()} exceeds yfinance max date {yfinance_max_date.date()}. Adjusting to {yfinance_max_date.date()}.")
            end_date = yfinance_max_date

        logging.info(f"Requested data for {self.symbol} from {start_date.date()} to {end_date.date()} with cutoff {cutoff_date.date()}")

        # Generate CSV if it doesn't exist
        if not os.path.exists(historical_file):
            logging.info(f"CSV file {historical_file} not found. Generating from yfinance.")
            stock = Ticker(self.symbol)
            for attempt in range(3):
                try:
                    data = stock.history(start=start_date.tz_convert(None).strftime('%Y-%m-%d'),
                                         end=min(end_date, yfinance_max_date).tz_convert(None).strftime('%Y-%m-%d'))
                    if data.empty:
                        logging.warning(f"No data returned from yfinance for {self.symbol} during CSV generation.")
                        break
                    data.index = data.index.tz_localize('UTC')
                    data.to_csv(historical_file, index_label="Date")
                    logging.info(f"Generated {historical_file} for {self.symbol}")
                    break
                except Exception as e:
                    logging.warning(f"Attempt {attempt + 1} failed to generate CSV for {self.symbol}: {str(e)}")
                    if attempt == 2:
                        logging.error(f"Failed to generate CSV for {self.symbol} after 3 attempts: {str(e)}")
                    time.sleep(1)
        else:
            logging.info(f"CSV file {historical_file} found at {absolute_file_path}")
        ...
        try:
            ...
            if historical_data.index.empty:
                raise ValueError(f"Historical data file ({historical_file}) is empty.")
            ...
            logging.info(f"CSV data range for {self.symbol}: {historical_start.date()} to {historical_end.date()}")

            # Case 1: Entire requested period is within CSV data and before or at cutoff
            if start_date >= historical_start and end_date <= historical_end and end_date <= cutoff_date:
                filtered_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= end_date)]
                if not filtered_data.empty:
                    logging.info(f"Using historical CSV data for {self.symbol} from {start_date.date()} to {end_date.date()}")
                    return filtered_data
                else:
                    raise ValueError(f"No data available in {historical_file} for {start_date.date()} to {end_date.date()}.")

            # Case 2: Requested period extends beyond cutoff or CSV data
            else:
                # Use CSV data up to cutoff or historical end
                csv_end = min(historical_end, cutoff_date)
                csv_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= csv_end)] if start_date <= csv_end else pd.DataFrame()

                # Fetch yfinance data from csv_end + 1 day to end_date if needed
                yf_start = csv_end + pd.Timedelta(days=1) if csv_end >= start_date else start_date
                if yf_start <= end_date:
                    logging.info(f"Fetching yfinance data for {self.symbol} from {yf_start.date()} to {end_date.date()}")
                    yf_data = None
                    for attempt in range(3):  # Retry up to 3 times
                        try:
                            yf_data = yf.download(self.symbol, start=yf_start.tz_convert(None).strftime('%Y-%m-%d'),
                                                  end=end_date.tz_convert(None).strftime('%Y-%m-%d'), progress=False)
                            if yf_data.empty:
                                raise ValueError(f"No data returned from yfinance for {self.symbol} between {yf_start.date()} and {end_date.date()}.")
                            yf_data.index = yf_data.index.tz_localize('UTC')
                            break
                        except Exception as e:
                            logging.warning(f"Attempt {attempt + 1} failed for yfinance fetch: {str(e)}")
                            if attempt == 2:
                                raise ValueError(f"Failed to fetch yfinance data for {self.symbol} between {yf_start.date()} and {end_date.date()} after 3 attempts: {str(e)}")
                            time.sleep(1)  # Wait before retrying
                else:
                    yf_data = pd.DataFrame()
                    logging.info(f"No yfinance fetch needed for {self.symbol}; CSV covers up to {csv_end.date()}")

                # Combine CSV and yfinance data
                if not csv_data.empty and not yf_data.empty:
                    combined_data = pd.concat([csv_data, yf_data]).drop_duplicates().sort_index()
                ...
                if filtered_data.empty:
                    raise ValueError(f"No combined data available for {self.symbol} between {start_date.date()} to {end_date.date()}.")
                logging.info(f"Using combined data (CSV up to {csv_end.date()}, yfinance from {yf_start.date()} to {end_date.date()}) for {self.symbol}")
                return filtered_data

        except (FileNotFoundError, ValueError) as e:
            logging.error(f"CSV error for {self.symbol}: {str(e)}. Falling back to yfinance for entire range.")
            yf_data = None
            for attempt in range(3):  # Retry up to 3 times
                try:
                    yf_data = yf.download(self.symbol, start=start_date.tz_convert(None).strftime('%Y-%m-%d'),
                                          end=end_date.tz_convert(None).strftime('%Y-%m-%d'), progress=False)
                    if yf_data.empty:
                        raise ValueError(f"No data returned from yfinance for {self.symbol} between {start_date.date()} and {end_date.date()}.")
                    yf_data.index = yf_data.index.tz_localize('UTC')
                    break
                except Exception as e:
                    logging.warning(f"Attempt {attempt + 1} failed for yfinance fetch (full range): {str(e)}")
                    if attempt == 2:
                        raise ValueError(f"Failed to fetch yfinance data for {self.symbol} between {start_date.date()} and {end_date.date()} after 3 attempts: {str(e)}")
                    time.sleep(1)  # Wait before retrying

            filtered_data = yf_data[(yf_data.index >= start_date) & (yf_data.index <= end_date)]
            if filtered_data.empty:
                raise ValueError(f"No data available from yfinance for {self.symbol} between {start_date.date()} and {end_date.date()}.")
            logging.info(f"Using yfinance data for {self.symbol} from {start_date.date()} to {end_date.date()}")
            return filtered_data
    ...
    def decompose_time_series(self, model='multiplicative', period=None, plot=True):
        if period is None:
            ...
            logging.error("No period specified for decomposition")
            return None, None
        if len(self.data['Close']) < 2 * period:
            st.error(f"Insufficient data for decomposition. Need at least {2 * period} observations, got {len(self.data['Close'])}.")
            logging.error(f"Insufficient data for decomposition: {len(self.data['Close'])} observations")
            return None, None
        try:
            ...
            if plot:
                ...
                plt.tight_layout()
                logging.info(f"Generated decomposition plot for {self.symbol}")
                return result, fig
            return result, None
        except Exception as e:
            st.error(f"Error during decomposition: {e}")
            logging.error(f"Decomposition failed for {self.symbol}: {str(e)}")
            return None, None
    ...
    def evaluate_arima_model(self, series, arima_order):
        ...
        for t in range(len(test)):
            try:
                ...
                logging.error(f"ARIMA{arima_order} evaluation failed for {self.symbol}")
                return float("inf")
    ...
    def evaluate_sarima_model(self, series, order, seasonal_order):
        ...
        train, test = series[:train_size], series[train_size:len(series)-100]
        try:
            model = SARIMAX(train, order=order, seasonal_order=seasonal_order, maxiter=50)
            model_fit = model.fit(disp=False)
            predictions = model_fit.predict(start=train_size, end=len(series)-101, dynamic=True)
            rmse = sqrt(mean_squared_error(test, predictions))
            logging.info(f"SARIMA{order}{seasonal_order} RMSE={rmse:.3f} for {self.symbol}")
            return rmse
        except:
            logging.error(f"SARIMA{order}{seasonal_order} evaluation failed for {self.symbol}")
            return float("inf")
    ...
    def evaluate_tslm(self, series, seasonal_period):
        ...
        try:
            ...
            rmse = sqrt(mean_squared_error(test, predictions))
            logging.info(f"TSLM RMSE={rmse:.3f} for {self.symbol}")
    ...
    def grid_search_arima(self, p_values, d_values, q_values):
        ...
        def evaluate_order(order):
            try:
                ...
                logging.info(f"Evaluated ARIMA{order} with RMSE={rmse:.3f} for {self.symbol}")
        ...
        progress.empty()
        logging.info(f"Best ARIMA order for {self.symbol}: {best_order} with RMSE={best_score:.3f}")
...
end_date = st.date_input("End Date", value=pd.to_datetime("2025-05-03").replace(hour=0, minute=0, second=0, microsecond=0))  # Set to current date
...
if st.button("Run Analysis"):
    ...
    if start_date_ts >= end_date_ts:
        ...
        try:
            analysis = StockMarketAnalysis(symbol, start_date, end_date, cutoff_date="2024-12-31")
            ...
            with tab4:
                ...
                try:
                    ...
                    st.write(f"Debug: Data length = {len(analysis.data['Close'])}, Train size = {int(len(analysis.data['Close']) * 0.8)}")
                except Exception as e:
                ...
                try:
                    ...
                    st.write(pd.DataFrame(model_comparison).round(6))
                except Exception as e:
