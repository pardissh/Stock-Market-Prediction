...
class StockMarketAnalysis:
    ...
    def determine_sarima_period(self, data_length, default_period):
        ...
        fallback_periods = [
            ('yearly', default_period),
            ('quarterly', max(63, default_period // 4)),
            ('monthly', max(21, default_period // 12))
        ]
    ...
    def decompose_time_series(self, model='multiplicative', period=None, plot=True):
        ...
        # Define fallback periods: quarterly (~63 trading days), monthly (~21 trading days)
        fallback_periods = [
            ('quarterly', max(63, period // 4)),  # Ensure at least 63 for quarterly
            ('monthly', max(21, period // 12))    # Ensure at least 21 for monthly
        ]
    ...
    def evaluate_arima_model(_self, series, arima_order):
        # Limit data to last 3 years (approx. 756 trading days) for faster computation
        max_data_points = 756
        if len(series) > max_data_points:
            series = series[-max_data_points:]
            logging.info(f"Reduced ARIMA data to last {max_data_points} points for {_self.symbol}")
        ...
        try:
            ...
            logging.info(f"ARIMA{arima_order} RMSE={rmse:.3f} for {_self.symbol}")
            return rmse
        except Exception as e:
    ...
    def evaluate_sarima_model(_self, series, order, seasonal_order, _period):
        ...
        max_data_points = 756
        if len(series) > max_data_points:
            series = series[-max_data_points ENTRY_1]
            logging.info(f"Reduced SARIMA data to last {max_data_points} points for {_self.symbol}")
        ...
        try:
            model = SARIMAX(
                ...
                maxiter=50,  # Increased for better convergence
                method='lbfgs'  # More robust solver
            )
            model_fit = model.fit(disp=False)
            predictions = model_fit.predict(start=len(train), end=len(train) + len(test) - 1, dynamic=True)
            rmse = sqrt(mean_squared_error(test, predictions))
            logging.info(f"SARIMA{order}{seasonal_order} RMSE={rmse:.3f} for {_self.symbol}")
            return rmse
        except Exception as e:
    ...
    def grid_search_arima(self, p_values, d_values, q_values):
        ...
        # Use multiprocessing only for larger grids to avoid overhead
            ...
            with tab4:
                ...
                with st.spinner("Evaluating SARIMA model (this may take a moment)..."):
                    try:
                        ...
                        sarima_rmse = analysis.evaluate_sarima_model(
                            analysis.data['Close'],
                            order=best_order,
                            seasonal_order=(0, 0, 0, period),
                            _period=period
                        )
                        if np.isinf(sarima_rmse):
                            st.error(f"SARIMA{best_order}(0,0,0)[{period}] failed to fit the data.")
                            logging.error(f"SARIMA{best_order}(0,0,0)[{period}] returned inf RMSE for {symbol}")
                        else:
                ...
                try:
                    ...
                    model_comparison = {
                        ...
                        "RMSE": [
                            arima_rmse if not np.isinf(arima_rmse) else "Failed",
