...
class StockMarketAnalysis:
    ...
    def load_data(self):
        ...
        try:
            ...
            if start_date >= historical_start and end_date <= historical_end and end_date <= pd.Timestamp.now(tz='UTC'):
                filtered_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= end_date)]
                if not filtered_data.empty:
                    logging.info(f"Using historical data for {self.symbol} from {start_date.date()} to {end_date.date()}")
                    return filtered_data
                else:
                    raise ValueError(f"No data available in {historical_file} for {start_date.date()} to {end_date.date()}.")
            else:
                logging.info(f"Date range {start_date.date()} to {end_date.date()} exceeds local data or future date. Fetching from yfinance.")
                yf_data = yf.download(self.symbol, start=start_date.tz_convert(None).strftime('%Y-%m-%d'), 
                                    end=min(end_date.tz_convert(None).strftime('%Y-%m-%d'), pd.Timestamp.now(tz=None).strftime('%Y-%m-%d')), progress=False)
                if yf_data.empty:
                ...
                if not historical_data.empty:
                    combined_data = pd.concat([historical_data, yf_data]).drop_duplicates().sort_index()
                ...
                logging.info(f"Using yfinance data (with historical supplement) for {self.symbol} from {start_date.date()} to {end_date.date()}")
                return filtered_data
    ...
    def calculate_yearly_trading_days(self):
        ...
        for year in range(start_year, end_year + 1):
            ...
            end_date = pd.Timestamp(f'{year}-12-31', tz=tz)  # Fixed typo in original
    ...
    def evaluate_sarima_model(self, series, order, seasonal_order):
        train_size = int(min(len(series) * 0.8, 500))  # Limit to 500 data points for speed
        train, test = series[:train_size], series[train_size:len(series)-100]  # Leave some data for validation
        try:
...
end_date = st.date_input("End Date", value=pd.to_datetime("2025-05-02"))  # Adjusted to current date
