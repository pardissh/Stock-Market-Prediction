...
class StockMarketAnalysis:
    ...
    def evaluate_arima_model(_self, series, arima_order):
        logging.info(f"Hash of input series: {hash(str(series.values))}")
    ...
    def grid_search_arima(self, series, p_values, d_values, q_values):
        ...
        def evaluate_order(order):
            try:
                rmse = self.evaluate_arima_model(series, order)
                train_size = int(len(series) * 0.8)
                test_size = len(series) - train_size
                logging.info(f"Evaluated ARIMA{order} with RMSE={rmse:.6f}, train_size={train_size}, test_size={test_size} for {self.symbol}")
...
if st.button("Run Analysis"):
    ...
    if start_date_ts >= end_date_ts:
        ...
        try:
            ...
            # Preprocess data once for consistency
            data_close = analysis.data['Close'].interpolate().ffill().bfill()
            st.write(f"Missing values in data['Close'] before preprocessing: {analysis.data['Close'].isna().sum()}")
            st.write(f"Missing values in data['Close'] after preprocessing: {data_close.isna().sum()}")
            ...
            with tab4:
                ...
                if st.session_state.grid_search_status == "not_started":
                    ...
                    with st.spinner("Performing ARIMA grid search..."):
                        try:
                            best_order, best_score, results = analysis.grid_search_arima(data_close, p_values, d_values, q_values)
                ...
                with st.spinner("Evaluating TSLM model..."):
                    try:
                        tslm_rmse, _ = analysis.evaluate_tslm(data_close, seasonal_period=63)
