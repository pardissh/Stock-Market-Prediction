...
class StockMarketAnalysis:
    ...
    def load_data(self):
        ...
        # First try to load from CSV without any yfinance fallback
        try:
            ...
            filtered_data = historical_data.loc[self.start_date:self.end_date]
            if not filtered_data.empty:
                logging.info(f"Successfully loaded data from CSV for {self.symbol}")
                return filtered_data
        except Exception as e:
            logging.warning(f"Failed to load from CSV: {str(e)}")
        
        # If CSV loading fails, only then try yfinance
        logging.info("Falling back to yfinance...")
        for attempt in range(3):
            try:
                yf_data = yf.download(self.symbol, 
