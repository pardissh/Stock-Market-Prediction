...
class StockMarketAnalysis:
    ...
    def load_data(self):
        # Define the cutoff date for historical data and make it UTC-aware
        historical_cutoff = pd.to_datetime("2025-04-30").tz_localize('UTC')
        ...
        # Load pre-saved historical data for the specific symbol
        try:
            ...
            # Ensure the index is a DatetimeIndex and UTC-aware
            ...
            raise ValueError(f"Historical data file ({historical_file}) not found. Please add it to the project directory.")

        # Filter historical data based on user start date
        historical_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= historical_cutoff)]

        # Fetch recent data if end_date is after the cutoff (only 1 or 2 days)
        if end_date > historical_cutoff:
            days_difference = (end_date - historical_cutoff).days
            if days_difference > 2:
                st.warning(f"Only up to 2 days beyond {historical_cutoff.date()} are supported. Using data up to {historical_cutoff.date()}.")
                return historical_data
            max_attempts = 3
            for attempt in range(max_attempts):
                try:
                    stock = yf.Ticker(self.symbol, session=None)
                    recent_data = stock.history(start=historical_cutoff, end=end_date)
                    if recent_data.empty:
                        st.warning(f"No recent data found for {self.symbol} from {historical_cutoff.date()} to {end_date.date()}. Using last available historical data.")
                        return historical_data.iloc[-1:]
                    # Ensure recent_data index is a DatetimeIndex and UTC-aware
                    if not isinstance(recent_data.index, pd.DatetimeIndex):
                        recent_data.index = pd.to_datetime(recent_data.index)
                    if recent_data.index.empty:
                        st.warning(f"Recent data is empty for {self.symbol}. Using last available historical data.")
                        return historical_data.iloc[-1:]
                    if recent_data.index.tz is None:
                        recent_data.index = recent_data.index.tz_localize('UTC')
                    else:
                        recent_data.index = recent_data.index.tz_convert('UTC')
                    # Concatenate and remove duplicates based on index
                    combined_data = pd.concat([historical_data, recent_data]).sort_index().loc[start_date:end_date]
                    combined_data = combined_data[~combined_data.index.duplicated(keep='last')]
                    logging.info(f"Successfully loaded and combined data for {self.symbol}")
                    return combined_data
                except Exception as e:
                    if "Too Many Requests" in str(e) and attempt < max_attempts - 1:
                        time.sleep(2 ** attempt)  # Exponential backoff
                        continue
                    st.warning(f"Recent data fetch failed: {str(e)}. Using last available historical data.")
                    logging.error(f"Failed to fetch recent data for {self.symbol}: {str(e)}")
                    return historical_data.iloc[-1:]
        else:
            # If end_date is within historical period, return filtered historical data
            logging.info(f"Using historical data only for {self.symbol}")
