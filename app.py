        ...
        try:
            ...
            # Case 2: Requested period extends beyond cutoff or CSV data
                ...
                if filtered_data.empty:
                    raise ValueError(f"No combined data available for {self.symbol} between {start_date.date()} and {end_date.date()}.")
            ...
            logging.info(f"CSV error for {self.symbol}: {str(e)}. Falling back to yfinance for entire range.")
            yf_data = None
            for attempt in range(3):  # Retry up to 3 times
                try:
