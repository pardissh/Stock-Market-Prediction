...
class StockMarketAnalysis:
    ...
    def evaluate_arima_model(self, series, arima_order):
        ...
        logging.info(f"Evaluating ARIMA{arima_order} for {self.symbol}, data length: {len(series)}")
        ...
        try:
            ...
            logging.info(f"ARIMA{arima_order} RMSE={rmse:.6f} for {self.symbol}, train_size={train_size}, test_size={len(test)}")
            return rmse
        except Exception as e:
            logging.error(f"ARIMA{arima_order} evaluation failed for {self.symbol}: {str(e)}")
            return float("inf")

    def evaluate_sarima_model(self, series, order, seasonal_order, _period):
        series = series.interpolate().ffill().bfill()
        data_length = len(series)
        fallback_periods = [('quarterly', 63), ('monthly', 21)]
        sarima_period = self.determine_sarima_period(data_length, _period)
        ...
        train, test = series[:train_size], series[train_size:]
        if len(test) == 0:
            logging.error(f"Empty test set for SARIMA{order}{seasonal_order} for {self.symbol}")
            return float("inf")
        try:
            model = SARIMAX(
                train,
            ...
            rmse = sqrt(mean_squared_error(test, predictions))
            logging.info(f"SARIMA{order}{seasonal_order} RMSE={rmse:.6f} for {self.symbol}")
            return rmse
        except Exception as e:
            logging.error(f"SARIMA{order}{seasonal_order} evaluation failed for {self.symbol}: {str(e)}")
        return float("inf")
    ...
    def evaluate_tslm(self, series, seasonal_period):
        ...
        train_size = int(len(series) * 0.8)  # Use full 80% of data
        train, test = series[:train_size], series[train_size:]
        ...
        try:
            ...
            rmse = sqrt(mean_squared_error(test, predictions))
            logging.info(f"TSLM RMSE={rmse:.6f} for {self.symbol}, train_size={train_size}, test_size={len(test)}")
            return rmse, model_fit
        except Exception as e:
            logging.error(f"TSLM evaluation failed for {self.symbol}: {str(e)}")
            return float("inf"), None
...
symbol = st.selectbox("Select Stock Symbol", stock_symbols, index=stock_symbols.index('AAPL'))

start_date = st.date_input("Start Date", value=pd.to_datetime("2023-02-02"))
end_date = st.date_input("End Date", value=pd.to_datetime("2025-02-15"))
forecast_steps = st.slider("Forecast Steps (Days)", min_value=5, max_value=60, value=30, step=5)

st.write("*Note*: Default values are set to match the analysis period for AAPL (2023-02-02 to 2025-04-30).")
            ...
            with tab4:
                ...
                with st.spinner("Evaluating SARIMA model (this may take a moment)..."):
                    try:
                        ...
                        if np.isinf(sarima_rmse):
                            ...
                            st.write(f"**Interpretation**: SARIMA extends ARIMA with seasonal components. A quarterly period (63 trading days) captures potential seasonal patterns, such as those driven by quarterly earnings reports. A lower RMSE indicates a good fit.")
                    except Exception as e:
                ...
                with st.spinner("Evaluating TSLM model..."):
                    try:
                        ...
                        st.write(f"**Interpretation**: TSLM uses a linear model with trend and seasonal dummies, aiming to capture long-term growth and quarterly cycles (63 trading days). Its simplicity may lead to poor performance (high RMSE) for stock prices, which exhibit complex, non-linear dynamics.")
                    except Exception as e:
...
st.write("- Deployment: 24 hours")
st.write("Total Time Spent: 112 hours")
st.write("**Resources**:")
