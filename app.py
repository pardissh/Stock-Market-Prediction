...
class StockMarketAnalysis:
    ...
    def determine_sarima_period(self, data_length, default_period):
        ...
        fallback_periods = [
            ...
            ('quarterly', 63),  # Fixed to 63 for quarterly
            ('monthly', 21)     # Fixed to 21 for monthly
        ]
        for period_type, period in fallback_periods:
            if period is not None and data_length >= 2 * period:
        ...
        if data_length < 2 * period:
            for period_type, fallback_period in fallback_periods:
                if data_length >= 2 * fallback_period:
                    st.warning(
                        f"Insuffiient data for decomposition with period {period} "
    ...
    def evaluate_tslm(self, series, seasonal_period):
        # Use a more reasonable period for TSLM
        data_length = len(series)
            ...
            with tab4:
                ...
                with st.spinner("Evaluating SARIMA model (this may take a moment)..."):
                    try:
                        ...
                        sarima_rmse = analysis.evaluate_sarima_model(
                            analysis.data['Close'],
                            order=best_order,
                            seasonal_order=(1, 0, 1, 63),  # Use non-zero seasonal order for quarterly period
                            _period=252
                        )
                        if np.isinf(sarima_rmse):
                            ...
                            st.write(f"SARIMA{best_order}(1,0,1)[63] RMSE: {sarima_rmse:.3f}")
                            st.write(f"**Interpretation**: SARIMA extends ARIMA with seasonal components. A quarterly period (63 days) captures potential seasonal patterns. A lower RMSE suggests a good fit.")
                    except Exception as e:
                ...
                with st.spinner("Evaluating TSLM model..."):
                    try:
                        ...
                        st.write(f"TSLM (trend + 63 seasonal dummies) RMSE: {tslm_rmse:.3f}")
