...
class StockMarketAnalysis:
    def __init__(self, symbol, start_date, end_date, cutoff_date="2025-04-30"):
        self.symbol = symbol
    ...
    def load_data(self):
        ...
        yfinance_max_date = pd.Timestamp("2025-04-30").tz_localize('UTC')
        ...
        logging.info(f"Requested data for {self.symbol} from {start_date.date()} to {end_date.date()}")
        ...
        if os.path.exists(historical_file):
            try:
                ...
                if start_date >= historical_start and end_date <= historical_end:
                    filtered_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= end_date)]
                    if not filtered_data.empty:
                        logging.info(f"Using CSV data for {self.symbol} from {start_date.date()} to {end_date.date()}")
                        return filtered_data
                    else:
                        raise ValueError(f"No data in CSV for {self.symbol} from {start_date.date()} to {end_date.date()}.")
                else:
                    logging.warning(f"CSV date range {historical_start.date()} to {historical_end.date()} does not cover {start_date.date()} to {end_date.date()}. Falling back to yfinance.")
            except Exception as e:
                logging.error(f"Failed to load CSV {historical_file}: {str(e)}")

        logging.info(f"Fetching yfinance data for {self.symbol} from {start_date.date()} to {end_date.date()}")
        for attempt in range(3):
            try:
                ...
                return yf_data[(yf_data.index >= start_date) & (yf_data.index <= end_date)]
            except Exception as e:
...
if st.button("Run Analysis"):
    ...
    elif end_date_ts.date() > current_date:
        st.error(f"End date cannot be in the future. Current date: {current_date}")
        logging.error(f"End date {end_date_ts.date()} is in the future")
    else:
        try:
            analysis = StockMarketAnalysis(symbol, start_date, end_date, cutoff_date="2025-04-30")
