...
class StockMarketAnalysis:
    ...
    # Replacement for load_data method
    def load_data(self):
        ...
        if end_date > current_date:
            ...
            logging.warning(f"End date adjusted to current date {current_date.date()} to avoid future data request.")
        ...
        try:
            ...
            logging.info(f"Local data range: {historical_start.date()} to {historical_end.date()}")
            ...
            # Case 2: Requested period extends beyond cutoff or CSV data
                ...
                if yf_start < end_date:
                    ...
                    try:
                        ...
                        if yf_data.empty:
                            raise ValueError(f"No data available from yfinance for {self.symbol} between {yf_start.date()} and {end_date.date()}.")
                        yf_data.index = yf_data.index.tz_localize('UTC')
                    except Exception as e:
                        raise ValueError(f"Failed to fetch yfinance data for {self.symbol} between {yf_start.date()} and {end_date.date()}: {str(e)}")
                else:
            ...
            logging.info(f"Local file error or insufficient data for {self.symbol}. Fetching from yfinance: {str(e)}")
            try:
                yf_data = yf.download(self.symbol, start=start_date.tz_convert(None).strftime('%Y-%m-%d'),
                                      end=end_date.tz_convert(None).strftime('%Y-%m-%d'), progress=False)
                if yf_data.empty:
                    raise ValueError(f"Failed to fetch data from yfinance for {self.symbol} between {start_date.date()} and {end_date.date()}.")
                yf_data.index = yf_data.index.tz_localize('UTC')
                ...
                raise ValueError(f"Failed to fetch yfinance data for {self.symbol} between {start_date.date()} and {end_date.date()}: {str(e)}")
