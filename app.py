...
class StockMarketAnalysis:
    ...
    def load_data(self):
        ...
        # Filter historical data based on user start and end dates
        filtered_data = historical_data[(historical_data.index >= start_date) & (historical_data.index <= end_date)]
        if filtered_data.empty:
            raise ValueError(f"No data available for {self.symbol} between {start_date.date()} and {end_date.date()}.")
