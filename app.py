...
class StockMarketAnalysis:
    ...
    def load_data(self):
        ...
        try:
            ...
            # Add Adj Close if missing by copying Close
            ...
            if start_date < historical_start or end_date > historical_end:
                raise ValueError(
                    f"Requested date range {start_date.date()} to {end_date.date()} "
                    f"is not fully covered by CSV data ({historical_start.date()} to {historical_end.date()})."
                )
